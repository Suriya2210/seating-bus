...
const Manager_seatlayout = () => {
  ...
  const BookSeats = async () => {
    ...
    selectedseat.forEach(async (element) => {
      ...
      var response = await axios.post("http://localhost:3000/api/auth/bookseat", json)

      if (response) {
        console.log("Manager seatlayout " + JSON.stringify(json));
        var book_res = await axios.post('http://localhost:3000/booking-success', response.data.data);
      }
      console.log("Manager layout response :" + JSON.stringify(response));
    });
